...
/**
 * Offers to parse an arrival rate trace into HLDLIM parameters. Uses the dlim.extractor.ModelExtractor.
	...
	protected boolean validatePage() {
		...
		if (seasonalPeriodText.isEnabled()) {
			try {
				...
				if (seasonalPeriod <= 0) {
					setMessage("Seasonal Period must be > 0.", IMessageProvider.ERROR);
					return false;
				}
		...
		if (offsetText.isEnabled()) {
			try {
				...
				if (offset < 0) {
					setMessage("First Burst Offset must not be negative.", IMessageProvider.ERROR);
					return false;
				}
			} catch (NumberFormatException e) {
		...
		if (seasonalsPerTrendText.isEnabled()) {
			try {
				...
				setMessage("Maximum Burst Value be an integer number.", IMessageProvider.ERROR);
				return false;
			}
		}		
