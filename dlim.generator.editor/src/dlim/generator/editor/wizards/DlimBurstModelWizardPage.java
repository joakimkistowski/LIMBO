...
public class DlimBurstModelWizardPage extends DlimModelWizardPage {
	...
	private Collection<String> getInitiaBurstNames() {
		...
		for (EClassifier eClassifier : DlimPackage.eINSTANCE.getEClassifiers()) {
			if (eClassifier instanceof EClass) {
				...
				if (!eClass.isAbstract() && DlimPackage.eINSTANCE.getBurst().isSuperTypeOf(eClass)) {
	...
	private void createDecompositionButton(Composite parent) {
		...
		toggleDecompositionButton.setText("Explicitly show Trend, Burst, and Noise Contributions in Plot");
	...
	protected boolean validatePage() {
		try {
			...
			if (period <= 0) {
				setMessage("Inter Burst Period must be > 0.", IMessageProvider.ERROR);
				return false;
			}
		} catch (NumberFormatException e) {
			setMessage("Inter Burst Period must be a number.", IMessageProvider.ERROR);
			return false;
		}
		...
		try {
			...
			if (offset <= 0) {
				setMessage("First Burst Offset must be > 0.", IMessageProvider.ERROR);
				return false;
			}
		} catch (NumberFormatException e) {
			setMessage("First Burst Offset must be a number.", IMessageProvider.ERROR);
			return false;
		}
		...
		try {
			...
			if (peak < 0) {
				setMessage("Maximum Burst Value must be >= 0.", IMessageProvider.ERROR);
				return false;
			}
		} catch (NumberFormatException e) {
			setMessage("Maximum Burst Value be a number.", IMessageProvider.ERROR);
			return false;
		}
		...
		try {
			...
			if (width < 0) {
				...
				setMessage("Burst Width must not be greater than the Inter Burst Period.", IMessageProvider.ERROR);
				return false;
			} else if (width != 0.0 && width > offset*2.0) {
				setMessage("Burst Width must not be greater than 2*Offset.", IMessageProvider.ERROR);
				return false;
			}
		} catch (NumberFormatException e) {
			setMessage("Maximum Burst Value be a number.", IMessageProvider.ERROR);
		...
		try {
			...
			setMessage("Noise Minimum Value must be a number.", IMessageProvider.ERROR);
			return false;
		}
		try {
			noiseMax = Double.parseDouble(noiseMaxText.getText().trim());
			if (noiseMax < noiseMin) {
				setMessage("Noise Maximum Value must be greater than Noise Minimum Value.", IMessageProvider.ERROR);
				return false;
			}
		} catch (NumberFormatException e) {
			setMessage("Noise Maximum Value must be a number.", IMessageProvider.ERROR);
			return false;
		}
	...
	private void populateModel() {
		//bursts
		rootSequence.getCombine().remove(burstCombinator);
		...
		try {
			if (width > 0 && peak > 0) {
				...
				burstContainerSequence.setTerminateAfterTime(rootSequence.getFinalDuration());
				...
				if (offset - width/2.0 == 0.0) {
					...
					burstContainerSequence.getSequenceFunctionContainers().add(offsetElement);
					TimeDependentFunctionContainer burstElement = factory.createTimeDependentFunctionContainer();
					burstElement.setName("bursts");
					burstElement.setDuration(rootSequence.getFinalDuration()-(offset - width/2.0));
					burstContainerSequence.getSequenceFunctionContainers().add(burstElement);
				...
				Burst burst = (Burst) factory.create((EClass)(DlimPackage.eINSTANCE.getEClassifier(shapeCombo.getText().trim())));
				burst.setBase(0.0);
				...
				rootSequence.getCombine().add(burstCombinator);
			}
		...
		//noise
		rootSequence.getCombine().remove(noiseCombinator);
		if (noiseMin != 0.0 || noiseMax != 0.0) {
			...
			rootSequence.getCombine().add(noiseCombinator);
		}
