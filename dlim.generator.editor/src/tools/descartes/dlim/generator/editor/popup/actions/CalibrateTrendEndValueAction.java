...
public class CalibrateTrendEndValueAction extends CalibrationAction {
	...
	public void run(IAction action) {
		...
		LaunchCalibrationDialog dialog = new LaunchCalibrationDialog(
				...
				+ ModelEvaluatorUtil.getFunctionDuration(trend), getShell(),
				this);
		dialog.open();
		if (!dialog.wasCanceled()) {
			...
			if (editor instanceof DlimEditor) {
				...
				SetCommand set = new SetCommand(dlimEditor.getEditingDomain(),
						trend,
						DlimPackage.eINSTANCE.getTrend_FunctionOutputAtEnd(),
						dialog.getNewValue());
	...
	public double executeCalibration(double desiredValue)
		...
		return Calibrator
				.calibrateTrendEndValue(desiredValue, trend, evaluator);
	}
