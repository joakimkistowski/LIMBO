...
public class AbstractionTest {
	...
	public static void main(String[] args) {
		if (args.length != 3 && args.length != 4) {
			System.out
					.println("USAGE:  java AbstractionTest timeSeries1 timeSeries2 radius [EuclideanDistance|ManhattanDistance|BinaryDistance]");
			System.exit(1);
		} else {
			...
			final PAA shrunkI = new PAA(tsI, (int) Math.round(Math
					.sqrt((double) tsI.size())));
			final PAA shrunkJ = new PAA(tsJ, (int) Math.round(Math
					.sqrt((double) tsJ.size())));
			final WarpPath coarsePath = com.dtw.DTW.getWarpPathBetween(shrunkI,
					shrunkJ, distFn);
			final WarpPath expandedPath = expandPath(coarsePath, shrunkI,
					shrunkJ);
			final SearchWindow w = new WarpPathWindow(expandedPath,
	...
	// Expand the small warp path ot the resolution fo tohe original time series
	// for tsI and tsJ.
